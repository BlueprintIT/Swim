...
class sql {
	...
	*/
		...
		if(is_resource($this->connId->connId) || is_object($this->connId->connId)){
			...
			if($this->query != ''){
				...
				if(!is_array($tabQuery)){
					...
					$this->query = $tabQuery;
					$result = $this->execQuery($tabQuery);
					$this->addChanges();
					return $result;
				} else {
					$time=0;
					...
					if(SQLITE3){
						$this->connId->connId->beginTransaction();
					} else {
						...
						if(!empty($query) && substr(trim($query), 0, 1)!='#'){
							if($this->_checkBeginQuery($query)){
								...
								if(isset($commitafter) && $commitafter){
									if(SQLITE3){
										$this->connId->connId->commit();
										$this->connId->connId->beginTransaction();
									} else {
										$this->execQuery('COMMIT TRANSACTION;');
										$this->execQuery('BEGIN TRANSACTION;');
									}
									$commitafter=false;
								}
					...
					if($error) {
						if(SQLITE3){
							$this->connId->connId->rollback();
						} else {
							$this->execQuery('ROLLBACK TRANSACTION;');
						}
					} else {
						if(SQLITE3){
							$this->connId->connId->commit();
	...
	function addChanges() {
		if(SQLITE3) {
			if(DEBUG) {
				$tempChanges = sqlitem_changes($this->connId->resId);
			} else {
				$tempChanges = @sqlitem_changes($this->connId->resId);
			}
		} else {
			if(DEBUG) {
				$tempChanges = sqlitem_changes($this->connId->connId);
			} else {
				$tempChanges = @sqlitem_changes($this->connId->connId);
			}
		}
	...
	function execQuery($query){
		...
		if(!eregi('^create[[:space:]]database', $queryExec)){
			...
			if($this->connId->getResId($queryExec)){
				...
				$this->errorMessage = '<table style="color: red;"><tr><td>'.$GLOBALS['traduct']->get(9).' '.$errorCode.' :</td><td>'.@sqlitem_error_string(((SQLITE3)? $this->connId->connId : $errorCode )).'</td></tr>';
			...
			if(!SQLiteExecFunction($GLOBALS['db'], $query)) {
				$error = true;
				$this->errorQuery = $query;
				$message .= '<li><span style="color: red; font-size: 11px">'.$GLOBALS['traduct']->get(100).'</span></li>';
			} else {
				if(DEBUG) $GLOBALS['dbsel'] = sqlitem_last_insert_rowid($GLOBALS['db']);
				else $GLOBALS['dbsel'] = @sqlitem_last_insert_rowid($GLOBALS['db']);
	...
	*/
	function getForm($query){
	  ...
	  echo '<!-- sql.class.php : getForm() -->'."\n";
		...
		echo '	<form name="sql" action="main.php?dbsel='.$GLOBALS['dbsel'].'&table='.$GLOBALS['table'].'" method="POST" ENCTYPE="multipart/form-data">
				<table class="Insert" cellspacing="0" cellpadding="0">
        <thead><tr><td class="Browse" colspan="2">
				'.$GLOBALS['traduct']->get(66).'</b>
        </td></tr>
        </thead>
				<tr><td class="DisplayQuery"><div>
        <textarea wrap="hard" name="DisplayQuery" cols="80" rows="'.(TEXAREA_NB_ROWS-$mozIE).'" onfocus="document.sql.DisplayQuery.select();">'.$this->cleanup($query).'</textarea>
				...
				'.$GLOBALS['traduct']->get(67).' :
				<input type="file" size="35" class="file" name="sqlFile">	
				<input type="hidden" name="action" value="sql">
				</div>
				</td>';
		if(!empty($GLOBALS['table'])) {
			echo '<td align="center" style="padding-top: 1px;" valign="top">'.$this->GetColumnSelect();
		}
		echo '</td></tr></table>'."\n";
	...
	*/
	function checkAccessResult($query){
		...
		if(@sqlitem_fetch_string($res) != 'table') {
			return false;
