...
$results = $_STORAGE->query('SELECT Item.id FROM Item LEFT JOIN Sequence ON Item.id=Sequence.item WHERE ISNULL(Sequence.Item) AND section="'.$_STORAGE->escape($section->getId()).'" AND id!='.$root->getId().';');
while ($results->valid())
{
