...
{var input=document.createElement("input");input.setAttribute("type","text");input.setAttribute("id",getFieldId(basefield,position,field));input.setAttribute("name",getFieldName(basefield,position,field));container.appendChild(input);}
...
{var select=document.createElement("select");select.setAttribute("id",getFieldId(basefield,position,field));select.setAttribute("name",getFieldName(basefield,position,field));container.appendChild(select);for(var id in fielddata.options){var option=document.createElement("option");option.setAttribute("value",id);select.appendChild(option);option.innerHTML=fielddata.options[id];}}
function createCompoundField(basefield,position,field,container,fielddata)
{switch(fielddata.type)
{case'file':create_file_field(basefield,position,field,container,fielddata);break;case'item':create_item_field(basefield,position,field,container,fielddata);break;case'date':create_date_field(basefield,position,field,container,fielddata);break;case'optionset':create_option_field(basefield,position,field,container,fielddata);break;default:create_simple_field(basefield,position,field,container,fielddata);}}
function switch_simple_field(basefield,field,pos1,pos2,fielddata)
{var field1=document.getElementById(getFieldId(basefield,pos1,field));var field2=document.getElementById(getFieldId(basefield,pos2,field));var temp=field1.value;field1.value=field2.value;field2.value=temp;}
...
{switch(fielddata.type)
{case'file':switch_file_field(basefield,field,pos1,pos2,fielddata);break;case'item':switch_item_field(basefield,field,pos1,pos2,fielddata);break;case'date':switch_date_field(basefield,field,pos1,pos2,fielddata);break;default:switch_simple_field(basefield,field,pos1,pos2,fielddata);}}
function createCompoundRow(compound)
{var body=document.getElementById('tbody_'+compound.id);var rowcount=body.rows.length;var row=document.createElement('tr');body.appendChild(row);var cell;for(var i in compound.fields)
{cell=document.createElement('td');row.appendChild(cell);createCompoundField(compound.id,rowcount,i,cell,compound.fields[i]);}
cell=document.createElement('td');row.appendChild(cell);buttons="<a href=\"#\" onclick=\"moveCompoundRow(compound_"+compound.id+", this.parentNode.parentNode, true); return false\">";buttons+="<img alt=\"Move up\" title=\"Move up\" src=\""+CONTENT+"/icons/up-purple.gif\">";buttons+="</a>";buttons+="<a href=\"#\" onclick=\"moveCompoundRow(compound_"+compound.id+", this.parentNode.parentNode, false); return false\">";buttons+="<img alt=\"Move down\" title=\"Move down\" src=\""+CONTENT+"/icons/down-purple.gif\">";buttons+="</a>";buttons+="<a href=\"#\" onclick=\"deleteCompoundRow(compound_"+compound.id+", this.parentNode.parentNode); return false\">";buttons+="<img alt=\"Delete row\" title=\"Delete row\" src=\""+CONTENT+"/icons/delete-page-purple.gif\">";buttons+="</a>";cell.innerHTML=buttons;}
...
pos2=pos1-1;else
pos2=pos1+1;if((pos2>=0)&&(pos2<body.rows.length))
{switchCompoundRows(compound,pos1,pos2);}}
function selectDate(calendar,input)
{var field=document.getElementById(input);var dates=calendar.getSelectedDates();var date;if(dates.length==0)
date=Date.now();else
