	...
	*/
	function form(){
	  echo '<!-- SQLiteSelect.class.php : form() -->'."\n";
		...
		echo '<form name="select" action="main.php?dbsel='.$GLOBALS['dbsel'].'&table='.$this->table.'" method="POST">'."\n";
	  ...
	  if (!isset($tabData)) $tabData = array();
		...
		echo '	<tr><td style="text-align:left" colspan="10">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.displayPics('arrow_ltr.gif').'&nbsp;
				<a href="javascript:'."setCheckBox('select','showField',true);".'" class="Browse">'.$GLOBALS['traduct']->get(34).'</a>&nbsp;/&nbsp;
				<a href="javascript:'."setCheckBox('select', 'showField', false);".'" class="Browse">'.$GLOBALS['traduct']->get(35).'</a>';
		echo '	<tr><td colspan="4">&nbsp;</td><td class="TitleHeader" style="text-align:left">'.$GLOBALS["traduct"]->get(203).'</td></tr>'."\n";
		echo '	<tr><td colspan="3">&nbsp;</td>'."\n".
		'<td style="text-align:right" nowrap="nowrap">'.
		'<label for="AND">'.$GLOBALS["traduct"]->get(204).'<input type="radio" name="operSuppl" id="AND" value="AND" checked="checked"></label><br/>'.
		'<label for="OR">'.$GLOBALS["traduct"]->get(205).'<input type="radio" name="operSuppl" id="OR" value="OR"></td>'.
		'<td style="text-align:left"><textarea name="CondSuppl" cols="'.TEXTAREA_NB_COLS.'" rows="4"></textarea></td>
					</td>
				</tr>';
		if(isset($req)) {
