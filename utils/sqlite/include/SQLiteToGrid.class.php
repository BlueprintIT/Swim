...
class SQLiteToGrid {
	...
	function _showHeader(){
	  $out = '<!-- SQLiteToGrid.class.php : _showHeader() -->'."\n";		
		...
		if(isset($this->tabCaption) && !empty($this->tabCaption)) 
			$out .= "<caption align='".$this->tabCaption["align"]."'>".$this->tabCaption["content"]."</caption>";
		$out .= "\t<thead>\n\t\t<tr>\n";
		...
		while(list($index, $titleColonne) = each($this->title)) {
			...
			if(!is_array($this->hide) || (!isset($this->hide[$index]) || !$this->hide[$index])){
				...
				if($this->buttonStyle){
					...
					if($linkCond) $out .= "\t\t\t".'<td align="'.$align.'" class="'.$this->tabId.'" nowrap="nowrap"><a href="'.$this->getVar.'sort'.$this->tabId."=".$index.'" class="'.$this->tabId.'" style="border: 0px">'.$titleColonne.$infoSort.'</a></td>'."\n";
					else $out .= "\t\t\t<td align=\"center\" class=\"".$this->tabId."\" nowrap=\"nowrap\">".$titleColonne."</td>\n";
				}
			}
		}
		if(is_array($this->calcColumn)){
	  ...
	  $out = '<!-- SQLiteToGrid.class.php : _showTable() -->'."\n";	
		...
		foreach($this->data as $ligne){
			...
			while(list($index, $value) = each($ligne)){
				...
				if(is_array($this->calcColumn)) {
					foreach($this->calcColumn as $calcCol){
						if($calcCol['position'] == $index) $out .= "\t\t".'<td bgcolor="'.$localBgColor.'" '.((!empty($calcCol['align']))? 'align="'.$calcCol['align'].'"' : '' ).'nowrap="nowrap" width="10%" class="'.$this->tabId.'">'.$this->_formatCalc($ligne, $calcCol['format'], $pos ).'</td>'."\n";
					}
				}				
				if(!is_array($this->hide) || !isset($this->hide[$index]) || !$this->hide[$index]) $out .= "\t\t".'<td bgcolor="'.$localBgColor.'" '.(($this->align[$index])? 'align="'.$this->align[$index].'"' : '' ).'nowrap="nowrap" class="'.$this->tabId.'">'.((!empty($this->format[$index]))? $this->_formatCalc($ligne, $this->format[$index]) : $value ).'</td>'."\n";
			}
			if(is_array($this->calcColumn)) {
				foreach($this->calcColumn as $calcCol){
					if($calcCol["position"] > $this->nbColonne) $out .= "\t\t".'<td bgcolor="'.$localBgColor.'" '.((!empty($calcCol['align']))? 'align="'.$calcCol['align'].'"' : '' ).'nowrap="nowrap" width="10%" class="'.$this->tabId.'">'.$this->_formatCalc($ligne, $calcCol['format'], $pos ).'</td>'."\n";
				}
			}
			$out .= "\t</tr>\n";
			$pos++;
	...
	*/
	function _showNavigate(){
	  $out = '<!-- SQLiteToGrid.class.php : _showNavigate() -->'."\n";	
		$out .= "\t<tr class=\"navbarre\"><td colspan=\"".$this->_countVisibleColumn()."\" align=\"center\" class=\"".$this->tabId."\" nowrap=nowrap>\n";
	...
	*/
	function _fetchField(){
		...
		if($res = SQLiteExecFunction($this->SQLiteConnId, $queryLoc)){
			for($i=0 ; $i < SQLiteNumFieldsFunction($res) ; $i++){
				$title[] = SQLiteFieldNameFunction($res, $i);
			}
	...
	*/
	function _countRecord(){
		if(!isset($this->nbRecordQuery)){
			//if (eregi('JOIN|GROUP|LIMIT|PRAGMA', $this->query)) $qCount = false;
			//else $qCount = true;
			if(SQLITE3) {
				$qCount = preg_match('/^\s*(UPDATE|DELETE|INSERT|ALTER|JOIN|GROUP|LIMIT|PRAGMA)\s/i', $this->query) != 1;
			} else {
				$qCount = false;
			}
			if($qCount){
				if($res = SQLiteExecFunction($this->SQLiteConnId, $this->queryCount)){
					$this->nbRecordQuery = SQLiteStringFunction($res);
				} else $this->_sendError($GLOBALS['traduct']->get(117));
			} else {
				if($res = SQLiteExecFunction($this->SQLiteConnId, $this->query)){
					$this->nbRecordQuery = SQLiteNumFunction($res);
				} else $this->_sendError($GLOBALS['traduct']->get(117));
			}			
		}
		return $this->nbRecordQuery;
	...
	function _getRecord(){
		if(isset($GLOBALS['TableListImpact'])){
			...
			foreach($tableList as $tableImpact){
				if(!eregi('\.', $tableImpact)){
					$tempInfoTable = SQLiteArrayFunction($this->SQLiteConnId, 'PRAGMA table_info('.brackets(trim($tableImpact)).');');
		...
		if($res = SQLiteExecFunction($this->SQLiteConnId, $query)){
			unset($tabRecord);
			$tabRecord = array();
			while($ligne = SQLiteFetchFunction($res, SQLITE_NUM)){
				$tabRecord[] = $ligne;
			}				
		}
