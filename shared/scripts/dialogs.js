...
if (YAHOO.widget.Panel) {
	...
	BlueprintIT.dialog.Wait.dialog = new YAHOO.widget.Panel("loaddlg", {
			width: "240px",
			fixedcenter: true,
			underlay: "shadow",
			close: false,
	...
	BlueprintIT.dialog.Wait.show = function(message) {
		BlueprintIT.dialog.Wait.dialog.setHeader(message);
		BlueprintIT.dialog.Wait.dialog.setBody("<img src=\"/swim/shared/images/loading.gif\"/>");
		BlueprintIT.dialog.Wait.dialog.render(document.body);
		BlueprintIT.dialog.Wait.dialog.show();
	}
	
	BlueprintIT.dialog.Wait.hide = function() {
		BlueprintIT.dialog.Wait.dialog.hide();
	}
}
else {
	BlueprintIT.dialog.Alert.show = function(title, message, icon) {
