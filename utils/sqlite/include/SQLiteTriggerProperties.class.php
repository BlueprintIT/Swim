...
class SQLiteTriggerProperties {
	...
	*/
	function triggerExist($trigger){
		...
		if($res = 	$this->connId->getResId($query)){
			$triggerSQL = SQLiteStringFunction($res);
	...
	function saveProp(){
		...
		foreach($query as $req){
			...
			if(!$res){
				...
				$errorMessage .= $GLOBALS['traduct']->get(9).' '.$errorCode.' : '.@sqlitem_error_string(((SQLITE3)? $this->connId->connId : $errorCode ))."\n";
			}
		}
		displayQuery($queryDisplay);
		if(!empty($errorMessage)) displayError($errorMessage);
		if($GLOBALS['action']!='delete') $this->propView();
	...
	*/
	function propView(){
		...
		if(!$GLOBALS['workDb']->isReadOnly() && displayCondition('properties')) echo "<a href=\"main.php?dbsel=".$GLOBALS["dbsel"]."&trigger=".$this->trigger."&action=modify\" class='base'>".$GLOBALS['traduct']->get(14).'</a>';
		else echo '<span class="base"><i>'.$GLOBALS['traduct']->get(14).'</i></span>';
		echo str_repeat('&nbsp;', 10);
		if(!$GLOBALS['workDb']->isReadOnly() && displayCondition('del')) echo '<a href="main.php?dbsel='.$GLOBALS['dbsel'].'&trigger='.$this->trigger.'&action=delete" class="base">'.$GLOBALS['traduct']->get(15).'</a>';
	...
	*/
	function triggerEditForm(){
		...
		echo "<form name=\"triggerprop\" action=\"main.php?dbsel=".$GLOBALS["dbsel"]."\" method=\"POST\">";
		echo "	<table cellpadding=2 cellspacing=0>
					...
					<tr><td align='right' class='viewPropTitle'>".$GLOBALS['traduct']->get(57)." :</td><td class='viewProp'><div><div style='float: left'>
						...
						</select></div><div id='column' style='float: right'><input type=\"text\" name=\"ColumnList\" value=\"".(($tabTrigInfo["TriggerEvent"]=="UPDATE OF ")? $tabTrigInfo["ColumnList"] : "" )."\" size=40></div></div></td>
					...
					<tr><td align='right' class='viewPropTitle'>".$GLOBALS['traduct']->get(59)." :</td><td class='viewProp'><div><div style='float: left'>
						...
						</select></div><div id='condition' style='float: right'><input type=\"text\" name=\"ConditionList\" value=\"".(($tabTrigInfo["TriggerCondition"]=="WHEN ")? $tabTrigInfo["ConditionList"] : "" )."\" size=40></td>
					<tr><td align='right' class='viewPropTitle'>".$GLOBALS['traduct']->get(60)." :</td><td class='viewProp'><textarea name='TriggerStep' cols=60 rows=4>".trim($tabTrigInfo["TriggerStep"])."</textarea></td>
				</table>";
		...
		echo "</form></center>";
		echo "<script>checkColumn();checkCondition();</script>";
	...
	*/
	function getOn($selected){
		...
		$listOn = '';
		if($res = 	$this->connId->getResId($query)){			
			while($ligne = SQLiteFetchFunction($res, SQLITE_ASSOC)){
