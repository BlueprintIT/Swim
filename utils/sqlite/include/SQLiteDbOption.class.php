...
class SQLiteDbOption {
	...
	function SQLiteDbOption(&$connId){
		...
		switch($GLOBALS['option_action']){
			...
			case 'vacuum':
				...
				break;
			case 'save':
				$query[] = 'PRAGMA '.((SQLITE3)? '' : 'default_' ).'synchronous='.$_POST['synchro'].';';
				$query[] = 'PRAGMA default_cache_size='.$_POST['cache_size'].';';
				$query[] = 'PRAGMA '.((SQLITE3)? '' : 'default_' ).'temp_store='.$_POST['temp_store'].';';
	...
	*/
	function optionView(){
		...
		$res = $this->connId->getResId('PRAGMA '.((SQLITE3)? '' : 'default_' ).'synchronous;');
		...
		$res = $this->connId->getResId('PRAGMA '.((SQLITE3)? '' : 'default_' ).'temp_store;');
		...
		if($ModifPropOk) echo '		<a href="main.php?dbsel='.$GLOBALS['dbsel'].'&action=options&option_action=check" class="base">&nbsp;&raquo;&nbsp;'.$GLOBALS['traduct']->get(183).'</a><br/><br/>';
		else echo '		&nbsp;&raquo;&nbsp;<span class="tabprop"><i>'.$GLOBALS['traduct']->get(183).'</i></span><br/><br/>';
		if($ModifPropOk) echo '		<a href="main.php?dbsel='.$GLOBALS['dbsel'].'&action=options&option_action=vacuum" class="base">&nbsp;&raquo;&nbsp;'.$GLOBALS['traduct']->get(184).'</a>';
		...
		echo '  <table align="center" cellpadding="10" cellspacing="5">
				<tr><td colspan=2 align="center">
					<form name="formOption" action="main.php?dbsel='.$GLOBALS['dbsel'].'&action=options&option_action=save" method="POST">
	...
	*/	
	function attachDb(){
		...
		$tabDb = SQLiteArrayFunction($GLOBALS['db'], $query, SQLITE_ASSOC);
		$out.= '			<tr class="Browse">
		...
		foreach($tabDb as $dbInfo){
			if(!in_array($dbInfo['id'], $tabAttachId) && ($dbInfo['id']!=$GLOBALS['dbsel'])) $out.= '<option value="'.$dbInfo['id'].'">'.$dbInfo['name'].'</option>';
		}		
		$out.= '						</select>
