...
class SQLiteFunctionProperties {
	...
	*/
	function functionExist($function){
		...
		$res = SQLiteExecFunction($GLOBALS['db'], $query);
		if(SQLiteNumFunction($res)==1){
			$exist = false;
			while($ligne = SQLiteFetchFunction($res, SQLITE_ASSOC)) {
				$this->functionProperties = $ligne;
				$exist = true;
			}
			return $exist;
		} else return false;
	...
	*/
	function saveProp(){
		...
		$errorMessage = '';
		$res = SQLiteExecFunction($GLOBALS['db'], $queryDisplay);
		if(!$res){
		...
		$GLOBALS['redirect'] = "<script>parent.left.location='left.php?dbsel=".$GLOBALS['dbsel']."'; parent.main.location='main.php?dbsel=".$GLOBALS['dbsel']."'</script>";
	...
	*/
	function propView(){
	  echo '<!-- SQLiteFunctionProperties.class.php : propView() -->'."\n";
		...
		if(!$GLOBALS['workDb']->isReadOnly() && displayCondition('properties')) echo '<a href="main.php?dbsel='.$GLOBALS['dbsel'].'&function='.$this->function.'&action=modify" class="base">'.$GLOBALS['traduct']->get(14).'</a>';
		else echo '<span class="base"><i>'.$GLOBALS['traduct']->get(14).'</i></span>';
		echo str_repeat('&nbsp;', 10);
		if(!$GLOBALS['workDb']->isReadOnly() && displayCondition('del')) echo '<a href="main.php?dbsel='.$GLOBALS['dbsel'].'&function='.$this->function.'&action=delete" class="base">'.$GLOBALS['traduct']->get(15).'</a>';
	...
	*/
	function functEditForm(){
	  echo '<!-- SQLiteFunctionProperties.class.php : functEditForm() -->'."\n";
		...
		echo '<form name="functprop" action="main.php?dbsel='.$GLOBALS['dbsel'].'" method="POST" onSubmit="return subform();">';
		...
		echo "\t".'<tr><td align="right" class="viewPropTitle">'.$GLOBALS['traduct']->get(21).' :</td><td class="viewProp"><textarea name="FunctCode" cols="'.TEXTAREA_NB_COLS.'" rows="'.TEXAREA_NB_ROWS.'">'.$FunctProp['funct_code'].'</textarea></td>';
		echo "\t".'<tr><td align="right" class="viewPropTitle"><div id="Pfinal1">'.$GLOBALS['traduct']->get(22).' :</div></td><td class="viewProp"><div id="Pfinal2"><textarea name="FunctFinalCode" cols="'.TEXTAREA_NB_COLS.'" rows="4">'.$FunctProp['funct_final_code'].'</textarea></div></td>';
