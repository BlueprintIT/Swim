...
class TableIndex {
	...
	*/
	function save(){
		...
		foreach($query as $req) {
			if(SQLITE3){
				if($req == "BEGIN TRANSACTION;") {
					$GLOBALS['workDb']->connId->beginTransaction();
					continue;
				}
				if($req == "COMMIT TRANSACTION;"){
					$GLOBALS['workDb']->connId->commit();
					continue;
				}
			}
			SQLiteExecFunction($GLOBALS['workDb']->connId, $req);
		}
		return;
	}
	...
	*/
	function indexPropView(){
		...
		echo '<form name="indexprop" action="main.php?dbsel='.$GLOBALS['dbsel'].'&table='.$this->table.'" method="POST">';
	...
	*/
	function linePropView($i, $info){
		...
		if(strtoupper($i) != 'PRIMARY'){		
			...
			if(!$GLOBALS['workDb']->isReadOnly() && displayCondition('properties')) echo '<a href="main.php?dbsel='.$GLOBALS['dbsel'].'&table='.$this->table.'&index_action=modify&indexSeq='.$i.'" class="Browse">'.displayPics('edit.png', $GLOBALS['traduct']->get(14)).'</a>';
			else echo '<i>'.displayPics('edit_off.png', $GLOBALS['traduct']->get(14)).'</i>';
			echo '</td>';
			echo '		<td align="center" class="Index">';
			if(!$GLOBALS['workDb']->isReadOnly() && displayCondition('properties')) 
				echo "<a href=\"javascript:if(confirm('".addslashes($GLOBALS['traduct']->get(92))." \'".$info['name']."\'')) parent.main.location='main.php?dbsel=".$GLOBALS['dbsel']."&table=".$this->table."&index_action=delete&indexSeq=".$i."';".'" class="Browse">'.
	...
	*/
	function formAddIndex(){
		echo '<form name="addIndex" action="main.php?dbsel='.$GLOBALS['dbsel'].'&table='.$this->table.'" method="POST">'."\n";
		echo '<span style="font-size: 12px;">'.$GLOBALS['traduct']->get(93).' <input type="text" class="text" name="nbCols" value=1 size="2" class="small-input" /> '.$GLOBALS['traduct']->get(94).' <input class="button" type="submit" value="'.$GLOBALS['traduct']->get(69).'" />';
	...
	*/
	function getIndexList(){
		...
		$tabIndexList = SQLiteArrayFunction($GLOBALS['workDb']->connId, 'PRAGMA index_list('.brackets($this->table).');', SQLITE_ASSOC);
		if(is_array($tabIndexList)) {
			foreach($tabIndexList as $indexInfo){
				if(eregi('autoindex', $indexInfo['name'])) continue;
				if(!empty($indexInfo)){
					$tabInfoIndex = SQLiteArrayFunction($GLOBALS['workDb']->connId, 'PRAGMA index_info('.brackets($indexInfo['name']).');', SQLITE_ASSOC);
