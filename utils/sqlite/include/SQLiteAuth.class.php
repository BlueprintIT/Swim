...
class SQliteAuth {
	...
	*/
	function getAuthParam(){
		...
		$infoUser = SQLiteArrayFunction($GLOBALS["db"], $query);
		if(empty($infoUser)) {
			$_SESSION['SQLiteManagerConnected'] = false;
			unset($_SESSION['SQLiteManagerUserId']);
			$_SESSION['oldUser'] = $_SERVER['PHP_AUTH_USER'];
	...
	*/
	function checkExistTable(){
		$existTables = SQLiteArrayFunction($GLOBALS['db'], "SELECT name FROM sqlite_master WHERE type='table' AND (name='users' OR name='groupes');", SQLITE_ASSOC);
		if(empty($existTables) || (count($existTables)!=2)) {
			...
			foreach($query as $req) SQLiteExecFunction($GLOBALS["db"], $req);
		}
		return;
	}
	...
	*/
	...
	/**
	* return groupe name for groupe ID
	*
	* @access public
	* param integer $groupe_id 
	*/
	function getGroupeName($groupe_id){
		$groupeName = SQLiteStringFunction('SELECT groupe_name FROM groupes WHERE groupe_id='.$groupe_id);
		return groupeName;
	}
	...
	function manageAuth(){
		...
		switch($GLOBALS['auth_action']){
			...
			case 'addUser':
				...
				break;
			case 'deleteUser':
				if($_REQUEST['user']!=1) SQLiteExecFunction($GLOBALS['db'], 'DELETE FROM users WHERE user_id='.$_REQUEST['user']);
				$this->viewPrivileges();
				break;
			case 'savePasswd':
				break;
			...
			case 'addGroupe':
				...
				break;
			case 'deleteGroupe':
				if($_REQUEST['groupe']!=1) SQLiteExecFunction($GLOBALS['db'], 'DELETE FROM groupes WHERE groupe_id='.$_REQUEST['groupe']);
				$this->viewPrivileges();
				break;
			case 'saveUser';
				if(!empty($_POST['name']) && !empty($_POST['login']) && !empty($_POST['groupe_id'])){
					...
					if(!empty($query)) SQLiteExecFunction($GLOBALS['db'], $query);
				}
				$this->viewPrivileges();
				break;
			case 'saveGroupe':
				if(!empty($_POST['groupe_name'])){
					...
					if(!empty($query)) {
						SQLiteExecFunction($GLOBALS['db'], $query);
					}
				}	
				$this->viewPrivileges();
				break;
	...
	*/
	function getUserInfo($user){
		if(isset($_POST) && isset($_POST["user"])){
			...
			$out = SQLiteArrayFunction($GLOBALS["db"], $query);
			return $out[0]; 
		}
	}
	...
	*/
	function formUser(){
		...
		$groupeList = SQLiteArrayFunction($GLOBALS["db"], "SELECT groupe_id, groupe_name FROM groupes");
	...
	*/
	function formGroupe(){
		...
		echo "<form name='groupe' method='POST' action=''>
				<table style='font-size: 10px'>
	...
	*/
	function changePasswd(){
		...
		if(isset($GLOBALS["passwd_action"]) && ($GLOBALS["passwd_action"] == "save")){
			...
			$passCurrent = SQLiteStringFunction($res);
		...
		if($error || !isset($GLOBALS["passwd_action"]) || ($GLOBALS["passwd_action"]=="")){
			echo "<form name='passwd' method=POST action=''>";
			...
			echo "<tr><td align='right' nowrap='nowrap'>".$GLOBALS["traduct"]->get(175)."</td><td>&nbsp;<input type='password' class='text' name='old' size=10></td></tr>";
			echo "<tr><td align='right' nowrap='nowrap'>".$GLOBALS["traduct"]->get(176)."</td><td>&nbsp;<input type='password' class='text' name='pass' size=10></td></tr>";
			echo "<tr><td align='right' nowrap='nowrap'>".$GLOBALS["traduct"]->get(177)."</td><td>&nbsp;<input type='password' class='text' name='confirm' size=10></td></tr>";
