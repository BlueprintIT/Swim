...
{style+=" iconnode_leaf";if(this.data.type)
...
BlueprintIT.widget.StyledTextNode.prototype.getStyle=function(){var style=YAHOO.widget.TextNode.prototype.getStyle.call(this);if(this.iconClass){var loc=(this.nextSibling)?"t":"l";var type="n";if(this.hasChildren(true)||this.isDynamic()){type=(this.expanded)?"m":"p";}
...
BlueprintIT.widget.DraggableTreeNodeProxy.prototype.applyConfig=function(){this.padding=[0,0,0,0];this.isTarget=false;this.maintainOffset=false;this.primaryButtonOnly=true;this.resizeFrame=false;this.centerFrame=false;this.scroll=false;this.setDragElId(YAHOO.util.DDProxy.dragElId);}
...
BlueprintIT.widget.DraggableTreeView.getNodeLabelId=function(node){if(node.labelElId)
return node.labelElId;if(node.contentElId)
return node.contentElId;if(node.getElId)
return node.getElId();return null;}
BlueprintIT.widget.DraggableTreeView.getNodeLabel=function(node){if(node.getLabelEl)
return node.getLabelEl();if(node.getContentEl)
...
if(node.nodeType==3){var text=node.nodeValue;if(text.search(/^\s*$/)==0){text=" ";if(nodes.length==0){text=null;}}
if(text){if((nodes.length>0)&&(typeof nodes[nodes.length-1]=="string")){nodes[nodes.length-1]+=text;}
else{nodes.push(text);}}}
node=node.nextSibling;}
var newnode;if(nodes.length==1){if(typeof nodes[0]=="string"){newnode=new YAHOO.widget.TextNode({label:nodes[0]},treenode,true);}
else if(nodes[0].tagName.toLowerCase()=="a"){newnode=new YAHOO.widget.TextNode({href:nodes[0].href,label:nodes[0].innerHTML},treenode,true);}
else{newnode=new YAHOO.widget.HTMLNode({html:nodes[0].outerHTML},treenode,true,true);}}
else{var html="";var i=0;for(i=0;i<nodes.length;i++){if(typeof nodes[i]=="string"){html+=nodes[i];}
else{html+=nodes[i].outerHTML;}}
newnode=new YAHOO.widget.HTMLNode({html:html},treenode,true,true);}
var node=item.firstChild;while(node){if((node.nodeType==1)&&((node.tagName.toLowerCase()=="ul")||(node.tagName.toLowerCase()=="ol"))){this.loadNodeContents(newnode,node);}
node=node.nextSibling;}
newnode.expanded=newnode.hasChildren();}
BlueprintIT.widget.TreeViewLoader.prototype.loadNodeContents=function(treenode,list){var node=list.firstChild;while(node){if((node.nodeType==1)&&(node.tagName.toLowerCase()=="li")){this.loadNode(treenode,node);}
node=node.nextSibling;}}
